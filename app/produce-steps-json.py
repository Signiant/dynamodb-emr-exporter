...
parser.add_argument(
    ...
    type=str,
    default="",
    help="Number of Clusters to create. (tables will be split evenly among them)"
)
...
def main(region, filter, destination, writetput, readtput,
         spikedread, s3location, appname, excludes, clusters):
    ...
    if conn:
        ...
        for chunk in table_list_list:
            ...
            for table in table_desc_list:
                ...
                if int(table['read']) > 0:
                    myLog(
                        "Table uses provisioned capacity - need to add throughput spike and reset steps")
                    # Does this table have autoscaling enabled?
                    scalable_target_info = scalable_target_exists(
                        region, "table/" + table['name'], "dynamodb:table:ReadCapacityUnits")
                    if scalable_target_info is not None:
                        myLog("Table " + table['name'] +
                              " has autoscaling enabled")
                        autoscale_min_spike_read_capacity = scalable_target_info[0]['MinCapacity'] + int(
                            spikedread)
                        autoscale_min_reset_read_capacity = scalable_target_info[0]['MinCapacity']
                        myLog(
                            "Table " +
                            table['name'] +
                            " has a current AS min capacity of " +
                            str(autoscale_min_reset_read_capacity))

                    if spikedread is not None:
                        tputSpikeStep = generateThroughputUpdateStep(
                            table['name'],
                            "Spike",
                            s3ScriptPath,
                            autoscale_min_spike_read_capacity,
                            autoscale_min_spike_read_capacity,
                            table['write'],
                            region)
                    ...
                    myLog(
                        "Table uses on-demand capacity - no need for spike and reset throughput steps")

                tableExportStep = generateTableExportStep(
                    table['name'], tableS3Path, readtput)
                exportSteps.append(tableExportStep)

                if int(table['read']) > 0:
                    if spikedread is not None:
                        tputResetStep = generateThroughputUpdateStep(
                            table['name'],
                            "Reset",
                            s3ScriptPath,
                            table['read'],
                            autoscale_min_reset_read_capacity,
                            table['write'],
                            region)
                        exportSteps.append(tputResetStep)

                tableImportStep = generateTableImportStep(
                    table['name'], tableS3Path, writetput)
...
###########
def generateThroughputUpdateStep(
        tableName,
        stepName,
        s3Path,
        readtput,
        autoscale_min_throughput,
        writetput,
        region):
...
###########
def generateTableExportStep(
        tableName, s3Path, readtput, jarPath=None, classPath=None):
...
###########
def generateTableImportStep(
        tableName, s3Path, writetput, jarPath=None, classPath=None):
...
def listTables(conn):
    ...
    while moreTables:
        ...
        if LastEvaluatedTableName != '':
            table_list = conn.list_tables(
                ExclusiveStartTableName=LastEvaluatedTableName, Limit=100)
...
def scalable_target_exists(region, resource_id, scalable_dimension):
    ...
    myLog("Checking if scalable target exists for " +
          resource_id + " for dimension " + scalable_dimension)
